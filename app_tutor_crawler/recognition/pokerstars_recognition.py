...
class PokerStarsTableRecognizer(PokerTableRecognizer):
    ...
    def detect_table_cards(self):
        ...
        cards_name = self.detect_cards(separators, sort_bboxes_method, cards_coordinates, path_to_numbers,
                                       path_to_suits)
        return cards_name
    ...
    def detect_stack_number(self):
        ...
        contours, _ = cv2.findContours(binary_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        bounding_boxes = convert_contours_to_bboxes(contours, self.cfg['stack']['min_height'],
                                                    self.cfg['stack']['min_width'])
        bounding_boxes = sort_bboxes(bounding_boxes, method='left-to-right')
    ...
    def find_player_bet(self, player):
        ...
        bet_img = self.img[player_bet_location[1]: player_bet_location[3], player_bet_location[0]: player_bet_location[2]]
        binary_img = thresholding(bet_img, self.cfg['pot']['value_1'], self.cfg['pot']['value_2'])
        contours, _ = cv2.findContours(binary_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        bounding_boxes = convert_contours_to_bboxes(contours, self.cfg['pot']['min_height'],
                                                    self.cfg['pot']['min_width'])

...
class GameRecorder:
    ...
    def __init__(self, conf_path):
        ...
        self.column_names = ['game_id', 'round', 'player', 'position', 'action', 'number', 'stack_before',
    ...
    def recognize_image(self, recognizer):
        try:
            ...
            if table_cards != self.last_table_cards:
                if len(table_cards) == 0 and len(self.last_table_cards) == 5:
    ...
    def run(self, source, debug=False, save=False):
        count = 0
        if source == 'live':
            while True:
...
# game_recorder.run(source='live', save=True)
start = time.time()
game_recorder.run(source='images/my_hand_history/game1', debug=False)
